...
void configurations_t::validate_keys(options_t const& options) {
#if __has_include(<ranges>)
...
configurations_t::configurations_t(options_t const& options) :
	config_{options} {
	validate_keys(config_);
}

configurations_t::configurations_t(options_t&& options) :
...
configurations_t::configurations_t(std::filesystem::path const& path, options_t const& options) :
	config_{} {
...
configurations_t::configurations_t(std::filesystem::path const& path, options_t&& options) :
	config_{} {
...
void configurations_t::load_file(options_t& config, std::filesystem::path const& path) {
	// Opens the file
	std::ifstream ifs;
...
bool configurations_t::contains(std::string const& key) const noexcept {
...
configurations_t::keys() const {
	...
	std::transform(config_.cbegin(), config_.cend(), std::inserter(keys, keys.end()), [](auto const& a) { return a.first; });
	return keys;
#endif
}
		...
		auto const options = std::accumulate(args.begin(), args.end(), std::move(options_t{}), [](auto&& res, auto const& a) {
			if (a == "-" || ! a.starts_with("-")) {
				...
				if (std::cmatch result; std::regex_match(a.data(), result, option_re)) {
					...
					throw std::invalid_argument(std::string{a});
			}
			return std::move(res);
		});
#else
